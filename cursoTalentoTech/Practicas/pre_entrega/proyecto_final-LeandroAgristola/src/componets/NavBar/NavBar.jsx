...
import { useAuth } from '../context/AuthContext'; // <-- AÑADIR ESTA LÍNEA
import { useCart } from '../context/CartContext'; // Para el contador del carrito

function NavBar() {
  const { usuario, logout } = useAuth(); // <-- USAR AUTHCONTEXT
  const { carrito } = useCart();
  const navigate = useNavigate();

  const handleLogout = () => {
    logout();
    navigate('/'); // Redirige a home después del logout
  };
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
      <div className="container">
        <Link className="navbar-brand" to="/">Colchonera</Link>
